...
import type { LeetCodeSyncStatus } from "@/types";
...
export function useSyncLeetCode() {
  ...
  const [status, setStatus] = useState<LeetCodeSyncStatus | null>(null);
  ...
  async function requestCode(handle: string) {
    ...
    setError(null);
    try {
      const response = await requestLeetCodeVerification(handle);
  ...
  async function verify() {
    ...
    setError(null);
    try {
      const response = await verifyLeetCodeSync();
      if (response.success && response.data) {
        const statusResponse = await getLeetCodeSyncStatus();
  ...
  async function refresh() {
    ...
    setError(null);
    try {
      const response = await refreshLeetCodeSync();
      if (response.success && response.data) {
        const statusResponse = await getLeetCodeSyncStatus();
        setStatus(statusResponse.data ?? response.data.stats);
